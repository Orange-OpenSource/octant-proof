...
(** *** 
    Any substitution that matches a clause [cl] (which 
    intersects with [Rv]) in the model of [p] has its restriction 
    to [Rv] in [subs]. This completeness condition of [subs]
...
- have Hsemb := Hxinit. 
  ...
  + move:Hxrec. 
    ...
    apply (Hm y Hy).
    destruct (bool_des_rew (#|tail_vars (body_cl ccl) :&: Rv| > 0)) as [Hclrv|Hclrv].
      ...
      - exists (scl (sub_filter s Rv) ccl).
        ...
        + apply/flattenP. exists (inst_rule ccl).
          - apply/map_f/Hclp.
          - unfold inst_rule. rewrite Hclrv. 
            ...
            rewrite mem_enum.
        apply (implyP (forallP (implyP (implyP (forallP subs_comp ccl) Hclp) Hclrv) _)).
        unfold bmatch. simpl. apply (match_body_bmatch def). 
        apply (subset_to_in Hsmatch (nomega_fp m Hclp)). reflexivity.
        (* need to show that s is a good way to instantiate ccl *)
        + apply/andP ; split ; trivial.
          have Hsub := (subsetP (match_body_incr ccl Hdedsub) s) Hsmatch.
      ...
      - exists ccl.
        + apply/flattenP. exists [:: ccl]. apply/mapP. exists ccl.
          apply Hclp. unfold inst_rule. by rewrite Hclrv.
          apply/mem_head.
        + apply/andP;split;auto.
          apply/imsetP. exists s;auto.
          apply ((subsetP (match_body_incr ccl Hdedsub) s) Hsmatch).
Qed.
...
- have Hsemb := Hxinit.
  ...
  + move:Hxrec. 
    ...
    apply/subsetP ; move => y Hy.
    apply (Hm y Hy).
    unfold inst_rule in Hcclinpinst.
    destruct (bool_des_rew (0 < #|tail_vars (body_cl ocl) :&: Rv|)) as [Hclrv|Hclrv];
    rewrite Hclrv in Hcclinpinst.
    - (* transub is the substitution that transformed the program *)
      ...
      + apply/val_inj/(Logic.eq_sym)/gr_raw_atom_scl_eq. 
        ...
        by right.
    - rewrite mem_seq1 in Hcclinpinst.
      exists s; [|by rewrite (eqP Hcclinpinst)].
      rewrite -(eqP Hcclinpinst).
